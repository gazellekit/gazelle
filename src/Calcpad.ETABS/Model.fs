...
module Model = 
    ...
    let private applyValidationChecks (predicates: seq<'T -> seq<ModelValidationResult>>) (input: 'T) : seq<ModelValidationResult> = 
    ...
    module StoreyGroup = 
        ...
        let create (s: UnvalidatedStoreyGroup) : StoreyGroup = 
            ...
            let slabVertices = s.SlabVertices |> Seq.map (fun (coord) -> { X = coord.[0] * 1.0<mm>; Y = coord.[1] * 1.0<mm> })
            let slabThickness = PositiveFloat.tryCreate s.SlabThickness

            // WARNING: need to add column validation!
            let columnCoords = 
                s.ColumnCoordinatePairs
                |> Seq.map (fun coords -> { X = coords.[0] * 1.0<mm>; Y = coords.[1] * 1.0<mm> })
        ...
        module Storeys = 
            ...
            let create (baseElevation: Elevation) (sg: seq<StoreyGroup>) : Storey list = 
                let rec createAll (baseElevation: Elevation) (s: seq<StoreyGroup>) (created: Storey list) : Storey list = 
                    let createForSingleGroup (baseElevation: Elevation) (sg: StoreyGroup) : (MasterStorey * seq<SimilarStorey>) = 
                        ...
                        let createAllSimilar (ids: seq<int>) (e: seq<Elevation>) (h: Height) (masterRef: MasterStorey) : seq<SimilarStorey> = 
                            (ids, e)
                            ||> Seq.map2 (fun id e -> 
                                { Name = Name $"Storey{id}"
                                  Elevation = e
        ...
        module Slabs = 
            ...
            let create (baseElevation: Elevation) (sg: seq<StoreyGroup>) : Slab list = 
                let rec createAll (baseElevation: Elevation) (sg: seq<StoreyGroup>) (created: Slab list) : Slab list =
                    ...
                    | currGroup::remainingGroups -> 
                        let slabs = 
                            createForSingleGroup baseElevation currGroup 
                            |> Seq.toList
                        let updatedElevation = 
                            slabs 
                            |> Seq.map (fun s -> s.Elevation)
                            |> Seq.max
        ...
        module Columns = 
            ...
            let create (baseElevation: Elevation) (sg: seq<StoreyGroup>) : Column list = 
                let rec createAll (baseElevation: Elevation) (sg: seq<StoreyGroup>) (created: Column list) : Column list =
                    let createForSingleGroup (baseElevation: Elevation) (sg: StoreyGroup) : seq<Column> = 
                        ...
                        let names = 
                            [for i in 1..1..columnStartAndEndPoints.Length -> 
                                Name $"Column{i}"]
                                   
                        let columns = 
                            (names, columnStartAndEndPoints)
                            ||> List.map2 (fun n p -> { Name = n; Start = p.Start; End = p.End })
                            |> List.map Column
                            |> Seq.ofList
                    ...
                    | currGroup::remainingGroups -> 
                        ...
                        let updatedElevation = 
                            cols 
                            |> List.map (fun (Column c) -> c.Start.Z)
                            |> List.max
                            |> Elevation
    ...
    let validate (b: BuildingBlueprint) : Result<Building, seq<ModelValidationResult>> = 
        ...
        let predicates = 
            seq { applyCheckToAll StoreyGroup.rangesArePositive
        ...
        let result = 
            b.StoreyGroups 
            |> applyValidationChecks predicates

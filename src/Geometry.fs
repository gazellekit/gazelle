...
type EndFacesFlag = IncludeEndFaces | ExcludeEndFaces

/// 2D coordinate set. 
type Point2D<[<Measure>] 'T> = 
    { X: float<'T>
      Y: float<'T> }
...
module Geometry = 
    ...
    module Create = 
        ...
        let cuboid (xyCentroid: Point2D<'T>) width depth length = 
            ...
            | _ -> 
                { StartPoint = { X = xyCentroid.X; Y = xyCentroid.Y; Z = 0.0<_> }
                  EndPoint =  { X = xyCentroid.X; Y = xyCentroid.Y; Z = 0.0<_> } 
                  Width = Width width 
                  Depth = Depth depth 

...
type UKBarSize = H6 | H8 | H10 | H12 | H16| H20 | H25 | H32 | H40 | H50
...
module Rebar = 
    ...
    module Steel =
        ...
        let getDensity (s: SteelGrade) =
            match s with UKSteelGrade _ -> Density 7850.0<kg/m^3>
    
        let getElasticModulus (s: SteelGrade) =
            match s with UKSteelGrade _ -> 200_000.0<N/mm^2>

    [<RequireQualifiedAccess>]
    module StraightBar =

        let tryCreate (s: SteelGrade) (b: BarSize) (centroid: Point2D<mm>) (l: Length<mm>) : StraightBar =
            let (Length length) = l 
    ...
    module RectangularLink =
        ...
        let tryCreate (s: SteelGrade) (b: BarSize) (centroid: Point3D<mm>) (w: Width<mm>) (d: Depth<mm>) =
            let (Width width) = w 
            let (Depth depth) = d

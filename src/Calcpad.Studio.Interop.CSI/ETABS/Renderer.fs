...
module Renderer =
    ...
    let private renderSlabs (sap: SAPModel) (slabs: Slab list) : unit =
        ...
        | V17 sap ->
            slabItems
            |> List.iter (fun s -> sap.AreaObj.AddByCoord(s.NumberOfPoints, s.X, s.Y, s.Z, s.Name) |> ignore)
        | V19 sap ->
            slabItems
            |> List.iter (fun s -> sap.AreaObj.AddByCoord(s.NumberOfPoints, s.X, s.Y, s.Z, s.Name) |> ignore)
    ...
    let private renderColumns (sap: SAPModel) (columns: Column list) =
        ...
        | V17 sap ->
            columnItems
            |> List.iter (fun c -> sap.FrameObj.AddByCoord(c.Xi, c.Yi, c.Zi, c.Xj, c.Yj, c.Zj, c.Name) |> ignore)
        | V19 sap ->
            columnItems
            |> List.iter (fun c -> sap.FrameObj.AddByCoord(c.Xi, c.Yi, c.Zi, c.Xj, c.Yj, c.Zj, c.Name) |> ignore)
